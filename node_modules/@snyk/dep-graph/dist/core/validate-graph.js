...
function validateGraph(graph, rootNodeId, pkgs, pkgNodes) {
    ...
    var reachableFromRoot = graphlib.alg.postorder(graph, [rootNodeId]);
    ...
    var pkgIds = _.keys(pkgs);
    var pkgsWithoutInstances = pkgIds
        .filter(function (pkgId) { return !pkgNodes[pkgId] || pkgNodes[pkgId].size === 0; });
    assert(pkgsWithoutInstances.length === 0, 'not all pkgs have instance nodes');
    var rootPkgId = graph.node(rootNodeId).pkgId;
    assert(pkgNodes[rootPkgId].size === 1, 'root pkg should have exactly one instance node');
}
